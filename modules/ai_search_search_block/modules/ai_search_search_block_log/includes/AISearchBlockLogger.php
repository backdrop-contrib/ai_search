...
class AISearchBlockLogger {
  ...
  public function start($block_id, $uid, $query) {
    ...
    $log_id = db_insert('ai_search_search_block_log')
      ->fields([
        'block_id' => $block_id,
        'uid' => $uid,
        'query' => $query,
  ...
  public function logResponse($id, $response) {
    db_update('ai_search_search_block_log')
      ->fields([
        'response' => $response,
        'changed' => REQUEST_TIME,
      ])

(function ($, Backdrop) {
  ...
  Backdrop.behaviors.searchApiAiSearchBlockLog = {
    attach: function (context, settings) {
      ...
      $('.ai_search_search_block_log_score', context).once('ai-log-score').each(function () {
        $(this).on('click', function () {
          var $score = $(this).data('aiSearchBlockLogScore');
          var $logId = null;
          
          // Get log ID from Backdrop settings
          if (typeof Backdrop.settings.ai_search_search_block !== 'undefined' && 
              typeof Backdrop.settings.ai_search_search_block.logId !== 'undefined') {
            $logId = Backdrop.settings.ai_search_search_block.logId;
          }
